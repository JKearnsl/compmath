...
from compmath_calc_server.models.slat.utils import transform_matrix, is_diagonal_dominance, normalize_matrix
...
def calc_zm(
    ...
    while True:
        ...
        x_prev = x.copy()
        for i in range(n):
            s = sum(a_matrix[i][j] * x_prev[j] for j in range(i) if j != i)
